...
class Translator {
    ...
    // 从localStorage加载配置
    loadConfig() {
        this.appId = localStorage.getItem('baiduAppId') || '';
        this.appKey = localStorage.getItem('baiduAppKey') || '';
    }

    // 设置百度API密钥
    setBaiduCredentials(appId, appKey) {
        this.appId = appId;
        this.appKey = appKey;
    ...
    // 使用本地代理服务器解决跨域问题
    async translateWithBaidu(word) {
        if (!this.appId || !this.appKey) {
            throw new Error('百度翻译API未配置');
        }

        const salt = Date.now().toString();
        const str = this.appId + word + salt + this.appKey;
        const sign = this.md5(str);

        try {
            // 直接调用百度翻译API（适用于Android环境）
            ...
            const apiUrl = `${baiduApiUrl}?${params.toString()}`;
            ...
            // 快速失败，让translate方法能够更快地切换到离线模式
            throw error;
        }
    }

    // 主翻译函数 - 先尝试本地词库，再尝试百度API
    async translate(word) {
        ...
        // 2. 尝试百度翻译API
        try {
            const baiduResult = await this.translateWithBaidu(word);
            return {
                word: word,
                meaning: baiduResult,
                source: 'baidu'
            ...
            console.warn('百度翻译失败，使用离线模式:', error);
            
            // 3. 如果百度API失败，返回提示信息
            return {
                word: word,
                meaning: '（需要配置百度翻译API或添加到本地词库）',
                source: 'none'
            };
        }
    }
